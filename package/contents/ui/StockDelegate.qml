...
Item {
    ...
    // ListModel roles (bound from parent delegate)
    ...
    property real w52Pct: 0
    property real fiftyTwoWeekHigh: 0
    property real fiftyTwoWeekLow: 0
    property real volume: 0
    property real avgVolume: 0
    ...
    height: {
        ...
        if (rootItem.showAth && !isAth && hasAth)
            h += 12
        if (rootItem.showPortfolio && portfolioText.length)
            h += 12
        if (rootItem.showDayRange && dayHigh > dayLow)
            h += 10
        if (rootItem.showPrePost && prePostText.length)
            h += 12
        if (rootItem.showEarnings && earningsText.length)
            h += 12
        return h
    }
    ...
    // Pulse flash when price updates
    Rectangle {
        anchors.fill: parent
        anchors.margins: 4
        radius: 12
    ...
    MouseArea {
        ...
        drag.target: rootItem && rootItem.sortMode === "aslisted" && !isGrid ? row : undefined
        ...
        onClicked: function(ev) {
            ...
            if (ev.button === Qt.MiddleButton)
                rootItem.refresh(true)
            else if (ev.button === Qt.RightButton)
                rootItem.pinToPanel(ticker)
            else
                rootItem.openSymbol(ticker)
        }

        onPressAndHold: {
            if (rootItem && rootItem.sortMode === "aslisted" && !isGrid)
        ...
        onReleased: {
            ...
            if (!rootItem || rootItem.sortMode !== "aslisted" || isGrid)
                return
            if (drag.active)
                rootItem.reorderFromDrag(ticker, row.y + row.height / 2)
        }
    }

    PlasmaCore.ToolTipArea {
        anchors.fill: parent
        mainText: ticker + (name ? " — " + name : "")
        subText: tipText
    }
    ...
    RowLayout {
        ...
        // Drag handle hint
        Text {
            visible: rootItem && rootItem.sortMode === "aslisted" && !isGrid
            text: "⋮⋮"
            color: muted
            font.pixelSize: 10
            opacity: 0.5
        ...
        // Sparkline chart — hidden when widget is narrow
        Canvas {
            id: sparkCanvas
